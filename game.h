 ...
 * leaf (^ 0), segment (| 1) corner (└ 2) and tee (┴ 3).
 * Note that a leaf directed Wesset_t (<) cannot be connected to its right
 ...
 *
 * @return the newly create* Observe the four types of pieces in their default direction (North):
 * leaf (^), segment (|) corner (└) and tee (┴).
 * Note that a leaf directed West (<) cannot be connected to its right
 * to another piece . This is also true for a segment directed North/South (|),
 * a tee to West (┤) and a corner directed South (┐) or West (┘).
 * Observe that in the solution all the pieces are connected in a network without loops.d game
 **/
